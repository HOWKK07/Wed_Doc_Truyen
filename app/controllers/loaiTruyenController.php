...
class LoaiTruyenController {
    ...
    public function themLoaiTruyen() {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $ten_loaitruyen = $_POST['ten_loaitruyen'];

            // Gọi model để thêm loại truyện
            $result = $this->model->themLoaiTruyen($ten_loaitruyen);

            if ($result) {
                header("Location: list.php?success=1");
                exit();
            } else {
                echo "Lỗi: Không thể thêm loại truyện.";
            }
        }
    }
    ...
    public function suaLoaiTruyen($id) {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $result = $this->model->capNhatLoaiTruyen($id, $ten_loaitruyen);

            if ($result) {
                header("Location: list.php?success=1");
                exit();
            } else {
                echo "Lỗi: Không thể cập nhật loại truyện.";
            }
        }
    }
    ...
    public function xoaLoaiTruyen($id) {
        $result = $this->model->xoaLoaiTruyen($id);

        if ($result) {
            header("Location: list.php?success=1");
            exit();
        } else {
            echo "Lỗi: Không thể xóa loại truyện.";
        }
    }
    ...
    // Phương thức để lấy thông tin loại truyện theo ID
